...
function endString(index, len) {
  return index == len - 1;
}

function findDigitToSumVee(textInput) {
  let sum = 0;
  const arr = []; // [first,last] //  1 5 7 => [1] => [1,5] => [1,7]
  let pushIndex = 0;

  let len = textInput.length;
  for (let i = 0; i < len; i++) {
    let c = textInput[i];
    let isNumber = isNum(c);
    let isNewLine = newLine(c);
    let isEndString = endString(c);

    if (isEndString && isNumber) arr[pushIndex] = +c;
    if (isNewLine || isEndString) {
      if (arr.length == 1) arr[1] = arr[0] + arr[0] * 10;
      console.log(arr);
      sum += arr.pop() + arr.pop() * 10;
      pushIndex = 0;
    } else if (isNumber && pushIndex == 0) {
      arr[pushIndex] = +c;
      pushIndex = 1;
    } else if (isNumber) {
      arr[pushIndex] = +c;
...
let r = findDigitToSumVee(textInput);
console.log(r);
