...
function findDigitToSumVee(textInput) {
  let sum = 0;
  const arr = []; // [first,last] //  1 5 7 => [1] => [1,5] => [1,7] => [7]
  let pushIndex = 0;
  ...
  for (let i = 0; i < len; i++) {
    let c = textInput[i];
    if (c == '\n' || i == len - 1) {
      if (i == len - 1 && isNum(c)) arr[pushIndex] = +c;
      if (arr.length == 1) sum += arr[0] + arr.pop() * 10;
      else sum += arr.pop() + arr.pop() * 10;
      pushIndex = 0;
    } else if (isNum(c) && pushIndex == 0) {
      arr[pushIndex++] = +c;
      arr[pushIndex];
    } else if (isNum(c)) {
      arr[1] = +c;
